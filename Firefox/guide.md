...
* Le but du script est de lancer Firefox et faire qu'un profil FF existant et utilisable, soit copié vers un Ramdisk en mode `CMD` minimisé, ou encore invisible (un .VBS non-testé) !. La contrainte est que windows restreint les fenêtres GUI lancé par cmd ou ps.
...
* donc 100% sûr car si un cas de collision ce produit il est réécrit 5mn plus tard, où bien à la fermeture immédiate de FF (écriture forcée).
...
pour cmd  (alternative minimisée).
```
